...
app.controller('UserController', function($scope, UserService, $location, $rootScope, $cookieStore) {
	...
	$scope.registerUser = function() {
		...
		UserService.registerUser($scope.user).then(function(response) {
			...
			/*
			 * error condition if status is 406 , either username is not valid /
			 * email is not valid display the error message in the
			 * registrationform
			 */
			 ...
			 * $scope.exception=response.data if($scope.error.code==2)
			 * $scope.duplicateUser=response.data if($scope.error.code==3)
	...
	if ($rootScope.currentUser != undefined) {
		UserService.getUser().then(function(response) {
			$scope.user = response.data // data is ntg but user object in json
										// format
		}, function(response) {
			console.log(response.status)
		})
